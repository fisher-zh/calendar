...
function Dianaday(objDate) {
    ...
    for (i = 1; i < 13 && offset > 0; i++) {
        if (leap > 0 && i == (leap + 1) && this.isLeap == false) {    // 闰月
            --i; this.isLeap = true; temp = getLeapMonthDays(this.year);
        }
        else {
...
function Calendar (y, m) {
    var solarDayObj, lunarDayObj, lY, lM, lD = 1, lL, lX = 0, tmp1, tmp2;
...
function CalendarDay () {
    ...
    // 节气 每个月有两个节气，所以月份的倍数即可
    tmp1 = solarTerm(year, month * 2) - 1;
    tmp2 = solarTerm(year, month * 2 + 1) - 1;
